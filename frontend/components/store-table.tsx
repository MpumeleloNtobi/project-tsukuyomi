...
export default function StoreTable() {
  ...
  const fetchStores = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/store`
      );
      if (!response.ok) {
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    try {
      ...
      if (result.success && result.updatedStore) {
        ...
        setStores(
          stores.map((store) => (store.id === id ? result.updatedStore : store))
        );
        ...
        toast.error(
          "Update seemed successful but data wasn't returned correctly."
        );
      ...
      toast.error(
        error.message || "There was a problem updating the store status"
  ...
  if (loading && stores.length === 0) {
    // Show loading only initially or if stores are empty
    return (
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleStatusChange(store.id, value)
                      }
                      disabled={
                        updatingId === store.id || editingId === store.id
                      } // Disable if editing name too
                    >
