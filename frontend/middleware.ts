  ...
  switch (userRole) {
    case "admin":
      ...
      if (isBuyerRoute(req) || isSellerRoute(req)) {
        ...
        console.log(
          "An admin cannot access buyer or seller routes. Redirecting ...:",
          url,
        );
        return NextResponse.redirect(url);
      }
    ...
    case "buyer":
      ...
      if (isAdminRoute(req) || isSellerRoute(req)) {
        ...
        console.log(
          "A buyer cannot access admin or seller routes. Redirecting ...:",
          url,
        );
        return NextResponse.redirect(url);
      }
    ...
    case "seller":
      ...
      if (isSellerSignUpRoute(req)) {
        ...
        console.log(
          "A seller cannot create more than one store Redirecting ...:",
          url,
        );
        return NextResponse.redirect(url);
      }
      ...
      if (isAdminRoute(req)) {
        const url = new URL("/seller/dashboard", req.url);
        console.log(
          "A seller cannot access admin routes. Redirecting ...:",
          url,
        );
        return NextResponse.redirect(url);
      }
    ...
    default:
      ...
      if (isAdminRoute(req) || isBuyerRoute(req) || isSellerRoute(req)) {
        const url = new URL("/home", req.url);
        console.log(
          "An unauthenticated user cannot access admin, buyer or seller routes. Redirecting ...:",
          url,
        );
        return NextResponse.redirect(url);
      }
