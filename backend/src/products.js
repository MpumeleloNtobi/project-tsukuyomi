...
const productsRoute = (app, dbUrl) => {
  ...
  app.get("/products", async (req, res) => {
    ...
    try {
      ...
      if (storeId) {
        ...
        products =
          await sql`SELECT * FROM products WHERE "storeId" = ${storeId};`;
      } else {
  ...
  app.put("/products/:product_id", async (req, res) => {
    ...
    if (image1url !== undefined) {
      if (typeof image1url !== "string")
        return res.status(400).json({ error: "image1url must be a string." });
      updates.image1url = image1url;
    }

    if (image2url !== undefined) {
      if (typeof image2url !== "string")
        return res.status(400).json({ error: "image2url must be a string." });
      updates.image2url = image2url;
    }

    if (image3url !== undefined) {
      if (typeof image3url !== "string")
        return res.status(400).json({ error: "image3url must be a string." });
